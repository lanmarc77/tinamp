...
#define UI_MAIN_STARTUPFLAG_IMAGE_PERSISTED 4
...
/**
  ...
  * 
  */
void SCREENS_pause0(uint16_t selectedFile,uint16_t amountOfFiles,char* folderName,uint16_t currentPlayMinute,uint8_t currentPlaySecond,uint8_t percent,uint32_t sleepTimeSecondsLeft){
    if(UI_ELEMENTS_isDisplayOff()) return;
    UI_ELEMENTS_cls();
...
/**
  ...
  * 
  */
void SCREENS_pause1(uint16_t selectedFile,uint16_t amountOfFiles,char* folderName,uint16_t currentPlayMinute,uint8_t currentPlaySecond,uint8_t percent,uint32_t sleepTimeSecondsLeft){
    if(UI_ELEMENTS_isDisplayOff()) return;
    UI_ELEMENTS_cls();
    UI_ELEMENTS_progressBar(percent);
...
void SCREENS_switchingOff(uint64_t sdSizeMB,uint8_t SPIFFSUsagePercent,int32_t numberOfBookmarks){
    ...
    sprintf(&s[0],"%7luMB",sdSizeMB);
    UI_ELEMENTS_textScrolly(3,2,12,&s[0]);
    sprintf(&s[0],"%6i/%d%%",numberOfBookmarks,SPIFFSUsagePercent);
...
/**
  * @brief shows the setup screen for setting the font
  *
  * @param number font number
  * @param blinkMode 1=numbers blink, 0=no blinking
  *
  */
void SCREENS_fontSetup(uint8_t number,uint8_t blinkMode){
    uint64_t now=UTIL_get_time_us();
    if(UI_ELEMENTS_isDisplayOff()) return;
    UI_ELEMENTS_cls();
    if(blinkMode==0){
        UI_ELEMENTS_mainSymbol(27);
    }else{
      if(((now/1000)%1000)<100){
      }else{
        UI_ELEMENTS_mainSymbol(27);
...
/**
  * @brief shows the setup screen for setting switching button a and b
  *
  * @param abSwitch 0=normal, 1=switched
  * @param blinkMode 1=numbers blink, 0=no blinking
  *
  */
void SCREENS_abSwitch(uint8_t abSwitch,uint8_t blinkMode){
  uint64_t now=UTIL_get_time_us();
  if(UI_ELEMENTS_isDisplayOff()) return;
  UI_ELEMENTS_cls();
  if(blinkMode==0){
      if(abSwitch==0){
        UI_ELEMENTS_mainSymbol(28);
      }else{
        UI_ELEMENTS_mainSymbol(29);
      }
  }else{
    if(((now/1000)%1000)<100){
    }else{
...
/**
  * @brief shows the setup screen for setting the screen saver mode
  *
  * @param mode 0=oled, 1=lcd with cleaning pattern
  * @param blinkMode 1=numbers blink, 0=no blinking
  *
  */
 void SCREENS_screenSaverSetup(uint8_t mode,uint8_t blinkMode){
  uint64_t now=UTIL_get_time_us();
  if(UI_ELEMENTS_isDisplayOff()) return;
  UI_ELEMENTS_cls();
  if(blinkMode==0){
      if(mode==0){
        UI_ELEMENTS_mainSymbol(30);
      }else{
        UI_ELEMENTS_mainSymbol(31);
      }
  }else{
    if(((now/1000)%1000)<100){
    }else{
